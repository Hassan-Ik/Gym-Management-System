...
$selectQuery=$connection->query("SELECT * FROM `trainer`"); 
  
?>
<a href="Add_Trainer.php"><button class="btn btn-warning my-3 float-left">Add Trainers</button></a>
<form class="search my-3 px-3 float-right" method="post">
  <input type="text" name="search" class="search-input" placeholder="Enter Client ID" >
...
<?php 
    ...
    if(isset($_POST["check"])){
      $search=$_POST["search"];
      $searchQuery=$connection->query("SELECT * FROM trainer WHERE trainer_id='".$search."'");
      if(empty($search)){ 
    ...
    <tr>
      ...
      <td><?=$data['trainer_contact'];?></td>
      <td><?=$data['trainer_photo'];?></td>
      <?php echo "<td><a href=action/Delete.php?id=".$data['trainer_id'].">Delete</a></td>"; ?>
      <?php echo "<td><a href=action/Update.php?id=".$data['trainer_id'].">Edit</a></td>"; ?>
  ...
  <?php endwhile;
    ...
    <tr>
    <td colspan="11"> 
    <?php echo "<h2>Your Data not Found</h2>" ?>
    </td>
    </tr>
