...
if (isset($_POST["submit"])) {
  ...
  $selectQuery=$connection->query("SELECT Password FROM `login` where login_id=2");
  $data=$selectQuery->fetch_assoc();
  
  if ($old=$data['Password'] && $new == $conf && !empty($old)) {
   $updateQuery=$connection->query("UPDATE `login`SET Password='".$new."' where login_id =2");
...
<div class="Add_Client_Div px-5 mx-5 py-5 ">
  ...
  <div class="form-group">
    ...
    <input type="password" name="oldpsd" class="form-control">
  </div>
  <div class="form-group">
    <label>New Password</label>
    <input type="password" name="newpsd" class="form-control">
  </div>
  <div class="form-group">
    <label>Confirm Password</label>
    <input type="password" name="conpsd" class="form-control">
  </div>
  <button type="submit" class="btn btn-warning mt-3" name="submit">Change Password</button>
</form>
</div>
